...
export const getBudgets = async (
  ...
  const rpcName =
    filters?.status === "active" ? "get_active_budgets" : "get_inactive_budgets";
  ...
  if (filters?.recurring && filters.recurring !== "all") {
    budgets = budgets.filter(
      (b) => (b.recurring ? "yes" : "no") === filters.recurring
    );
  }

  if (filters?.carryover && filters.carryover !== "all") {
    budgets = budgets.filter(
      (b) => (b.carry_over ? "yes" : "no") === filters.carryover
    );
  }
...
export const getBudgetSummary = async (
  userId: string,
  filters?: BudgetFilters): Promise<BudgetSummary> => {
  const {
    status = "active",
    category = "all",
    period = "all",
